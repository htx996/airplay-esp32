# ESP32-S3 Release 2026-05-27

适用板型：`ESP32-S3-N16R8`

## 文件说明

- `esp32s3-merged-16mb-20260527.bin`
  全量单文件包，包含 bootloader、分区表、OTA 数据、应用固件和 `SPIFFS` 网页资源。
- `esp32s3-ota-firmware-20260527.bin`
  应用 OTA 包，只更新程序本体，不更新网页资源。
- `esp32s3-spiffs-storage-20260527.bin`
  网页资源 OTA 包，只更新 `storage / SPIFFS` 分区，不更新程序本体。

## 什么时候用哪个

- 第一次升级到“支持网页资源 OTA”的版本：
  直接刷 `esp32s3-merged-16mb-20260527.bin` 最省事。
- 以后只改 C 代码、后端逻辑、AirPlay 行为：
  用 `esp32s3-ota-firmware-20260527.bin`
